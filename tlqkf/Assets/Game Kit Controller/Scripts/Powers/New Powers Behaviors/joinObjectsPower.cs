	...
	void FixedUpdate ()
	{
		...
		if (joinObjects) {
			...
			if (currentCheckCollisionType1 && currentCheckCollisionType2) {
				...
				if (particles1) {
					particles1.startSpeed = GKC_Utils.distance (joinParticles1.transform.position, joinObject2.position) / 2;
				}
				...
				if (particles2) {
					particles2.startSpeed = GKC_Utils.distance (joinParticles2.transform.position, joinObject1.position) / 2;
				}
