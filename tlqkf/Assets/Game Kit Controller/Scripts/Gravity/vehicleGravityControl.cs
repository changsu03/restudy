...
public class vehicleGravityControl : MonoBehaviour
{
	...
	public eventParameters.eventToCallWithVector3 setGravityNormalEvent;
	public eventParameters.eventToCallWithBool setGravityControlEnabledEvent;
	...
	void Start ()
	{
		...
		setGravityNormalEvent.Invoke (currentNormal);

		if (startWithNewGravity) {
			if (useVehicleRotation) {
				Vector3 normalDirection = -transform.up;
				if (adjustRotationToSurfaceFound) {
					if (Physics.Raycast (transform.position, -transform.up, out hit, Mathf.Infinity, settings.layer)) {
	...
	void Update ()
	{
		...
		if (searchingSurface) {
			...
			if (Physics.Raycast (pos, dir, out hit, settings.vehicleRadius, settings.layer)) {
				...
				if (!hit.collider.isTrigger && !hit.rigidbody) {
					...
					if (hit.collider.gameObject.tag == settings.tagForCircumnavigate) {
						circumnavigableSurfaceFound = true;
					}

					//if the surface is moving
					if (hit.collider.gameObject.tag == settings.tagForMovingObjects) {
					...
					setGravityControlEnabledEvent.Invoke (false);
		...
		if (!OnGround && !powerActivated) {
			...
			if (searchNewSurfaceOnHighFallSpeed && transform.InverseTransformDirection (mainRigidbody.velocity).y < -settings.velocityToSearch && !searchingNewSurfaceBelow && !powerActivated) {
				...
				if (currentNormal != regularGravity) {
					...
					setGravityControlEnabledEvent.Invoke (true);
				}
			}
		}
		...
		if (!searchingSurface && (circumnavigableSurfaceFound || fatherDetected) && recalculatingSurface) {
			...
			if (Physics.Raycast (settings.centerOfMass.transform.position, -currentNormal, out hit, distance, settings.layer)) {
			...
			if (checkSurfaceBelowLedge) {
				...
				gravityAdherenceRaycastParentAngleRotation = Vector3.SignedAngle (transform.forward, mainRigidbody.velocity, transform.up);
			...
			if (surfaceFound) {
				if (!currentSurfaceFound.collider.isTrigger && !currentSurfaceFound.rigidbody) {
					...
					if (currentSurfaceFound.collider.gameObject.tag == settings.tagForCircumnavigate) {
					...
					else if (currentSurfaceFound.collider.gameObject.tag == settings.tagForMovingObjects) {
						surfaceNormal = currentSurfaceFound.normal;
						if (!fatherDetected) {
		...
		if ((gravityPowerActive || circumnavigateCurrentSurfaceActive) && !powerActivated) {
			if (currentObjectBelowVehicle) {
				//check if the object detected can be circumnavigate
				if (currentObjectBelowVehicle.tag == settings.tagForCircumnavigate) {
					circumnavigableSurfaceFound = true;
				} else {
					circumnavigableSurfaceFound = false;
				}
				...
				if (currentObjectBelowVehicle.tag == settings.tagForMovingObjects) {
				...
				} else if (fatherDetected && !currentObjectBelowVehicle.transform.IsChildOf (fatherDetected.transform)) {
					removeParent ();
				}
				...
				if (previousSurfaceBelowVehicle) {
	...
	void FixedUpdate ()
	{
		...
		if (!powerActivated && !gravityForcePaused) {
			...
			if (Physics.Raycast (settings.centerOfMass.transform.position, -currentNormal, out hit, settings.rayDistance, settings.layer)) {
		...
		if (OnGround) {
			if (!onGroundChecked) {
				...
				if ((circumnavigableSurfaceFound || fatherDetected) && gravityPowerActive) {
					recalculatingSurface = true;
				} else {
					recalculatingSurface = false;
				}
	...
	public void deactivateGravityPower ()
	{
		...
		if (searchingSurface || searchingNewSurfaceBelow || searchAround || currentNormal != regularGravity) {
			...
			setGravityNormalEvent.Invoke (regularGravity);
	...
	public IEnumerator rotateToSurface (Vector3 normal, int rotSpeed)
	{
		...
		setGravityNormalEvent.Invoke (normal);
	...
	public IEnumerator rotateVehicleToLandSurfaceCoroutine (Vector3 hitNormal)
	{
		...
		//set the new normal in the vehicle controller
		setGravityNormalEvent.Invoke (hitNormal);
	...
	void OnDrawGizmosSelected ()
	{
		if (showGizmo) {
			...
			Gizmos.DrawLine (settings.centerOfMass.transform.position, settings.centerOfMass.transform.position - settings.centerOfMass.transform.up * settings.rayDistance);
		}
	}
