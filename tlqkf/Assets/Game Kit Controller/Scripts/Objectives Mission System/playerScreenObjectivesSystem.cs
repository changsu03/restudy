...
public class playerScreenObjectivesSystem : MonoBehaviour
{
	...
	void FixedUpdate ()
	{
		if (!showObjectivesActive || showObjectivesPaused || objectiveList.Count == 0) {
			return;
		}
		...
		for (int i = 0; i < objectiveList.Count; i++) {
			...
			if (currentObjective.mapObject && currentObjective.iconTransform) {
				currenMapObjectPosition = currentObjective.mapObject.transform.position;

				if (currentObjective.iconOffset > 0) {
					currenMapObjectPosition += currentObjective.mapObject.transform.up * currentObjective.iconOffset;
				}
				currentDistace = GKC_Utils.distance (player.position, currenMapObjectPosition);
				...
				if (currentObjective.showIconPaused) {
					...
					if (targetOnScreen) {
						...
						if (currentObjective.showDistance) {
							currentObjective.iconText.text = Mathf.Round (currentDistace).ToString ();
						}
					} 
					...
					//if the target is off screen, change the icon to an arrow to follow the target position and also rotate the arrow to the target direction
						...
						if (currentObjective.showOffScreenIcon) {
							...
							currentObjective.iconTransform.rotation = Quaternion.Euler (0, 0, angle * Mathf.Rad2Deg);
						} else {
							currentObjective.onScreenIcon.SetActive (false);
							currentObjective.iconText.gameObject.SetActive (false);
						}
	...
	public void removeElementFromList (int objectID)
	{
		for (int i = 0; i < objectiveList.Count; i++) {
			if (objectiveList [i].ID == objectID) {
				if (objectiveList [i].iconTransform) {
	...
	{
		if (objectiveListElement.iconTransform) {
	...
	public void addElementToPlayerList (GameObject obj, bool addMapIcon, bool useCloseDistance, float radiusDistance, bool showOffScreen, bool showMapWindowIcon, 
	                                    bool showDistanceInfo, string objectiveIconName, bool useCustomObjectiveColor, Color objectiveColor, 
	                                    bool removeCustomObjectiveColor, int buildingIndex, float iconOffset, bool addIconOnRestOfPlayers)
	{
		...
		for (int i = 0; i < screenObjectivesManager.objectiveIconList.Count; i++) {
			if (screenObjectivesManager.objectiveIconList [i].name == objectiveIconName) {
		...
		if (currentObjectiveIconIndex != -1) {
			...
			if (mapCreatorManager && addMapIcon && showMapWindowIcon) {
				mapCreatorManager.addMapObject (true, true, false, obj, "Objective", Vector3.zero, -1, -1, buildingIndex, 0, false, false, false, null, 0);
			}
