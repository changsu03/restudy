	...
	public void checkObjectDetected (Collider col)
	{
		if (canActivateEffect (col)) {
			...
			if (currentProjectileInfo.killInOneShot) {
				...
				applyDamage.checkHealth (projectileGameObject, objectToDamage, currentProjectileInfo.projectileDamage, -transform.forward, 
					transform.position, currentProjectileInfo.owner, false, true);
			}
