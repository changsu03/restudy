...
public class projectileSystem : MonoBehaviour
{
	...
	[HideInInspector] public GameObject projectileGameObject;
	[HideInInspector] public RaycastHit hit;
	[HideInInspector] public Rigidbody mainRigidbody;
	[HideInInspector] public bool projectileUsed;

	[HideInInspector] public GameObject bulletMesh;
	[HideInInspector] public GameObject secondaryBulletMesh;
	...
	TrailRenderer mainTrailRenderer;
	Collider mainCollider;
	MeshCollider secondaryBulletMeshCollider;

	vehicleHUDManager vehicleManagerParent;
	vehicleDamageReceiver vehicleDamageReceiverParent;
	health healthParent;
	characterDamageReceiver characterDamageReceiverParent;

	bool isVehicle;
	bool isVehicleDamageReceiver;
	bool isCharacterHealth;
	bool isCharacterDamageReceiver;
	...
	void Update ()
	{
		if ((currentProjectileInfo.isHommingProjectile || currentProjectileInfo.isSeeker) && !mainRigidbody.isKinematic) {
			...
			if (currentProjectileInfo.target) {
				currentTargetPosition = currentProjectileInfo.target.transform.position + (currentProjectileInfo.target.transform.up * targetPositionOffset);
				...
				if (orbitingTargetTime > 0.5f) {
					orbitingTargetTime = 0;
					rotationSpeed *= 2;
				}

				currentDistanceToTarget = GKC_Utils.distance (currentProjectileInfo.target.transform.position, currentProjectilePosition);
				...
				transform.rotation = Quaternion.Slerp (transform.rotation, currentRotation, Time.deltaTime * currentRotationSpeed);
			}

			if (projectileCaptured && !currentProjectileInfo.target) {
		...
		if (projectileUsed) {
			if (projectileAttachedToSurface) {
				if (isVehicle) {
					if (vehicleManagerParent.destroyed) {
						parentDestroyed ();
					}
				}

				if (isVehicleDamageReceiver) {
					if (vehicleDamageReceiverParent.isVehicleDestroyed ()) {
						parentDestroyed ();
					}
				}

				if (isCharacterHealth) {
					if (healthParent.dead) {
						parentDestroyed ();
					}
				}

				if (isCharacterDamageReceiver) {
					if (characterDamageReceiverParent.isCharacterDead ()) {
						parentDestroyed ();
					}
				}
			}
	...
	void FixedUpdate ()
	{
		if (currentProjectileInfo.useRaycastCheckingOnRigidbody && !projectilePaused && !projectileUsed) {
			...
			if (movementSqrMagnitude > sqrMinimumExtent) { 
				...
				if (Physics.Raycast (previousPosition, movementThisStep, out hit, movementMagnitude + 0.1f, currentProjectileInfo.targetToDamageLayer)) {
	...
	{
		if (secondaryBulletMeshCollider) {
		...
		if (canActivateEffect (col)) {
			...
			if (objectToDamage.layer == LayerMask.NameToLayer (armorSurfaceLayer)) {
				if (!projectileCaptured) {
					...
					projectileGameObject.transform.SetParent (objectToDamage.gameObject.transform.parent);
			...
			if (projectileUsed) {
				...
				if (!objectToDamageRigidbody && !currentProjectileInfo.useGravityOnLaunch && !currentProjectileInfo.useGraivtyOnImpact &&
			...
			if (currentCheckCollisionType) {
				currentCheckCollisionType.checkTriggerWithGameObject (projectileGameObject, true);
			}

			if (isNewGameObjectImpact) {
				//damage the surface where the proectile impacts
				if (currentProjectileInfo.killInOneShot) {
					applyDamage.killCharacter (projectileGameObject, objectToDamage, -transform.forward, transform.position, 
						currentProjectileInfo.owner, false);
				} else {
					bool damageMultipleTimes = false;
					...
					if (currentProjectileInfo.projectileDamage > 0) {
						applyDamage.checkHealth (projectileGameObject, objectToDamage, currentProjectileInfo.projectileDamage, -transform.forward, transform.position, 
							currentProjectileInfo.owner, damageMultipleTimes, true);
					}
	...
	public void checkMapIcon ()
	{
		if (GetComponent<mapObjectInformation> ()) {
			screenObjectivesSystem screenObjectivesSystemManager = FindObjectOfType <screenObjectivesSystem> ();
			screenObjectivesSystemManager.removeGameObjectFromList (projectileGameObject);
		}
	}
	...
	IEnumerator explodeProjectileCoroutine ()
	{
		...
		audioClipBip audioClipBipManager = GetComponentInChildren<audioClipBip> ();
		if (audioClipBipManager) {
	...
	void explodeProjectile ()
	{
		...
		if (!explosionActivatedd) {
			...
			applyDamage.setExplosion (currentPosition, currentProjectileInfo.explosionRadius, true, currentProjectileInfo.targetToDamageLayer, currentProjectileInfo.owner, 
				currentProjectileInfo.canDamageProjectileOwner, projectileGameObject, currentProjectileInfo.killInOneShot, currentProjectileInfo.isExplosive, currentProjectileInfo.isImplosive, 
				...
				currentProjectileInfo.explosionForceToVehiclesMultiplier, currentProjectileInfo.explosionForce, currentProjectileInfo.forceMode, false, currentProjectileInfo.owner.transform);
	...
	public void setProjectileScorch ()
	{
		...
		if (objectToDamage) {
			scorchCreatedCorrectly = decalManager.setImpactDecal (decalManager.checkIfHasDecalImpact (objectToDamage), projectileGameObject, objectToDamage,
		...
		if (!scorchCreatedCorrectly) {
			...
			if (currentProjectileInfo.scorch) {
				if (objectToDamage) {
					//the bullet fired is a simple bullet or a greanade, check the hit point with a raycast to set in it a scorch
					if (Physics.Raycast (transform.position - transform.forward * 0.7f, transform.forward, out hit, currentProjectileInfo.scorchRayCastDistance, currentProjectileInfo.targetForScorchLayer)) {
						decalManager.setScorch (transform.rotation, currentProjectileInfo.scorch, hit, objectToDamage);
					}
				}
			}
			creatImpactParticles ();
	...
	{
		GameObject newImpactSound = new GameObject ();
		newImpactSound.transform.position = transform.position;
		newImpactSound.AddComponent<AudioSource> ().clip = currentProjectileInfo.impactSoundEffect;
		...
		newImpactSound.AddComponent<destroyGameObject> ().setTimer (currentProjectileInfo.impactSoundEffect.length);
		newImpactSound.name = "Impact Sound";
		decalManager.setImpactSoundParent (newImpactSound);
	}

	public bool canActivateEffect (Collider col)
	{
		if (!projectileUsed && (1 << col.gameObject.layer & currentProjectileInfo.targetToDamageLayer.value) == 1 << col.gameObject.layer && !projectilePaused &&
	...
	{
		if (currentProjectileInfo.impactParticles && !impactParticlesCreated) {
			GameObject impactParticles = (GameObject)Instantiate (currentProjectileInfo.impactParticles, transform.position, transform.rotation);
	...
	public void checkSurfaceWithDelay (Collider surface)
	{
		...
		surfaceCoroutine = StartCoroutine (checkSurfaceWithDeayCoroutine (surface));
	}

	IEnumerator checkSurfaceWithDeayCoroutine (Collider surface)
	{
	...
	{
		if (fakeProjectileTrail) {
			fakeProjectileTrail.gameObject.SetActive (true);
	...
	void startBullet ()
	{
		...
		if (transform.childCount > 0) {
			...
			if (transform.childCount > 1) {
				...
				enableOrDisableSecondaryMeshCollider (false);
			}
		}
		...
		if (currentProjectileInfo.projectileParticles) {
	...
	{
		if (currentProjectileInfo.owner) {
			...
			if (placeToShoot) {
				...
				transform.LookAt (currentProjectileInfo.target.transform.position);
			}
		} else {
	...
	public void attachProjectileToSurface ()
	{
		if (Physics.Raycast (transform.position - transform.forward * 0.7f, transform.forward, out hit, currentProjectileInfo.scorchRayCastDistance, currentProjectileInfo.targetForScorchLayer)) {
			...
			bool attachedCorrectly = decalManager.placeProjectileInSurface (transform.rotation, gameObject, hit, objectToDamage, currentProjectileInfo.adhereToLimbs);
	...
	public void checkProjectilesParent ()
	{
		...
		if (transformParent) {
			vehicleManagerParent = transformParent.GetComponent<vehicleHUDManager> ();
			if (vehicleManagerParent == null) {
				vehicleDamageReceiverParent = transformParent.GetComponent<vehicleDamageReceiver> ();
				if (vehicleDamageReceiverParent == null) {
					healthParent = transformParent.GetComponent<health> ();
					if (healthParent == null) {
						characterDamageReceiverParent = transformParent.GetComponent<characterDamageReceiver> ();
						if (characterDamageReceiverParent == null) {

						} else {
							isCharacterDamageReceiver = true;
							//print ("character d");
						}
					} else {
						isCharacterHealth = true;
						//print ("character");
					}
				} else {
					isVehicleDamageReceiver = true;
					//print ("vehicle d");
				}
			} else {
				isVehicle = true;
	...
	{
		if (sendObjectDetectedOnImpactEvent && objectToDamage) {
