...
public class padlockSystem : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if (!unlocked && usingDevice && !mainGameManager.isGamePaused ()) {
//			//use the center of the camera as mouse, checking also the touch input
	...
	public void activatePadlock ()
	{
		...
		if (usingDevice) {
			...
			currentPadlockSystemPlayerManagement = currentPlayer.GetComponentInChildren<padlockSystemPlayerManagement> ();
