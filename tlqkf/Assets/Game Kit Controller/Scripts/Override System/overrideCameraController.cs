...
public class overrideCameraController : MonoBehaviour
{
	...
	void Update ()
	{
		if (cameraControllerEnabled && cameraActive && !mainGameManager.isGamePaused ()) {
			transform.position = targetToFollow.position;

			//get the current input axis values from the input manager
			axisValues = overrideInput.getCustomMovementAxis ("mouse");
	...
	public float checkCameraCollision ()
	{
		...
		ray.origin = pivotTransform.position;
