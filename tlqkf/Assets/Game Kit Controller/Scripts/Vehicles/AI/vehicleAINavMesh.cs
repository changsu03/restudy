...
public class vehicleAINavMesh : MonoBehaviour
{
	...
	movementInfo movementVehicleInfo = new movementInfo ();
	...
	void FixedUpdate ()
	{
		...
		if (m_Target == null || !m_Driving) {
			...
			movementVehicleInfo.steering = 0;
			movementVehicleInfo.accel = 0;
			movementVehicleInfo.footbrake = -1;
			movementVehicleInfo.handbrake = 1;
			SendMessage ("Move", movementVehicleInfo, SendMessageOptions.DontRequireReceiver);
		} else {
			...
			if (Time.time < m_AvoidOtherCarTime) {
				...
				(Mathf.PerlinNoise (Time.time * m_LateralWanderSpeed, m_RandomPerlin) * 2 - 1) *	m_LateralWanderDistance;
			}
			...
			movementVehicleInfo.steering = steer;
			movementVehicleInfo.accel = accel;
			movementVehicleInfo.footbrake = accel;
			movementVehicleInfo.handbrake = 0;
			SendMessage ("Move", movementVehicleInfo, SendMessageOptions.DontRequireReceiver);
	...
	void OnCollisionStay (Collision col)
	{
		...
		if (col.rigidbody != null) {
			var otherAI = col.rigidbody.GetComponent<vehicleAINavMesh> ();
			if (otherAI != null) {
