...
public class vehicleInterface : MonoBehaviour
{
		...
		if (interfaceEnabled) {
			if (useInterfacePanelInfoList) {
				if (interfacePanelSelected) {
					...
					if (movingPanel) {
						interfacePanelParent.localPosition += (interfacePanelParent.right * axisValues.x + interfacePanelParent.up * axisValues.y) * movePanelSpeed;
					}
	...
	{
		#if UNITY_EDITOR
		EditorUtility.SetDirty (GetComponent<vehicleInterface> ());
		#endif
	}
