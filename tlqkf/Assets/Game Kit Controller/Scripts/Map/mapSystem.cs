...
public class mapSystem : MonoBehaviour
{
	...
	public bool setMartOnCurrentFloor;
	...
	void Start ()
	{
		if (mapEnabled) {
			...
			if (!mapCreatorManager) {
				mapEnabled = false;
				print ("Map Creator hasn't been found, The map manager is disabled, along with the map window. Drag and drop the Map System (in the Map Prefabs folder) to create the level map");
			...
			for (int i = 0; i < floors.Count; i++) {
				if (floors [i].floor == null) {
					mapEnabled = false;
					print ("Map Creator has been found, but the floors/buldings haven't been assigned correctly, check the map system inspector and press the button Search Building List");
					mapContent.SetActive (false);
			...
			if (useMapIndexWindow) {
				...
				for (int i = 0; i < mapIconTypes.Count; i++) {
					...
					newGlossaryElement.textName = iconInfoElementClone.GetComponentInChildren<Text> ();
					newGlossaryElement.slider = iconInfoElementClone.GetComponentInChildren<Slider> ();
					newGlossaryElement.typeName = mapIconTypes [i].typeName;
			...
			zoomScrollbar.value = zoomWhenOpen / maxZoom;
	...
	public void setCompassInitialState ()
	{
		if (!compassEnabled) {
			...
			addCompassIcon (northGameObject, north.gameObject, compassOffset, false);
			addCompassIcon (southGameObject, south.gameObject, compassOffset, false);
			addCompassIcon (eastGameObject, east.gameObject, compassOffset, false);
			addCompassIcon (westGameObject, west.gameObject, compassOffset, false);

			if (showIntermediateDirections) {
				addCompassIcon (northEastGameObject, northEast.gameObject, compassOffset, false);
				addCompassIcon (southWestGameObject, southWest.gameObject, compassOffset, false);
				addCompassIcon (southEastGameObject, southEast.gameObject, compassOffset, false);
				addCompassIcon (northwestGameObject, northWest.gameObject, compassOffset, false);
			}
		}
		...
		if (miniMapWindowEnabledInGame || mapOpened) {
					...
					if (currentMapIcon) {
						...
						if ((currentMapObjectInfo.buildingIndex == currentBuildingIndex || currentMapObjectInfo.visibleAllBuildings) &&
							...
							if (currentMapObjectInfo.isActivated && canEnableCurrentIcon) {
								...
								currentMapIcon.sizeDelta = Vector2.Lerp (currentMapIcon.sizeDelta, new Vector2 (currenIconSize, currenIconSize), Time.deltaTime * changeIconSizeSpeed);

								mapIconRotation = Vector3.zero;
								if (currentMapObjectInfo.followCameraRotation) {
									mapIconRotation = mapCameraRotation;
								}
								...
								if (mapOpened && useMapCursor && mapCursor && showInfoIconInsideCursor) {
									float currentDistance = GKC_Utils.distance (mapCursorRectTransform.position, currentMapIcon.position);
								...
								if (useTextInIcons) {
									if (mapOpened && !changingCameraSize) {
										...
										if (currentCameraSize <= zoomToActivateTextIconsValue && !currentIconOffscreen) {
											...
											currentMapTextName.position = currentMapIcon.position + Vector3.up * (textIconsOffset.y * (currenIconSize / maxIconSize)) +
											Vector3.right * (textIconsOffset.x * (currenIconSize / maxIconSize));
						...
						print ("Map Icon for object " + currentMapObjectInfo.name + " wasn't found, make sure the map icon is configured correcly in the map creator");
					}
				} else {
					checkCurrentMapIconPressedParent ();

					if (mapObjects [i].mapIcon) {
			...
			if (mapOpened && useMapCursor && mapCursor && showInfoIconInsideCursor) {
				if (currentDistanceToMapCursor < maxDistanceToMapIcon) {
					if (currentMapObjectIndexOnMapCursor > -1 && mapObjects.Count > 0) {
	...
	public void checkCurrentMapIconPressedParent ()
	{
		if (useCurrentMapIconPressed) {
			if (currenMapIconPressed) {
				currenMapIconPressed.transform.SetParent (mapWindow);
				currenMapIconPressed.SetActive (false);
			}
		}
	}
	...
	void Update ()
	{
		...
		if (mapEnabled) {
			...
			if (!mapOpened) {
				...
				mapCamera.transform.position = Vector3.Lerp (mapCamera.transform.position, newCameraPosition, Time.deltaTime * playerIconMovementSpeed);
			}
...
//							}
							...
							mapCamera.transform.position -= moveInput * mouseDragMapSpeed * zoomAmountMultiplier;
						} else {
...
//						}
				...
				if (!mapViewIn2dActive) {
					mouseValues = playerInput.getPlayerMovementAxis ("mouse");
					...
					if ((horizontalInput != 0 || verticalInput != 0) && movingMap) {
						...
						mapSystemPivotTransform.localRotation = Quaternion.Slerp (mapSystemPivotTransform.localRotation, currentPivotRotation, Time.deltaTime * map3dRotationSpeed);
						mapSystemCameraTransform.localRotation = Quaternion.Slerp (mapSystemCameraTransform.localRotation, currentCameraRotation, Time.deltaTime * map3dRotationSpeed);
					}
				...
				if (!changingCameraSize) {
					...
					if (!usingScrollbarZoom) {
						...
						if (mapViewIn2dActive) {
							zoomScrollbar.value = currentCameraSize / (maxZoom - minZoom);
						} else {
							zoomScrollbar.value = currentCameraSize / (maxZoom3d - minZoom3d);
						}
					}
					...
					if (zoomingIn) {
						if (mapViewIn2dActive) {
							...
							currentCameraSize -= Time.deltaTime * zoomSpeed3d;
					...
					if (mapViewIn2dActive) {
						assignMapCameraOrthographicValue (Mathf.Lerp (currentOrthographicSize, currentCameraSize, Time.deltaTime * zoomSpeed));
					} else {
						assignMapCameraFovValue (Mathf.Lerp (currentFieldOfView, currentCameraSize, Time.deltaTime * zoomSpeed3d));
					}
				}

				currentFloorNumberText.text = (currentFloorNumber).ToString ();
			}
		}
	...
	public void updateCompassIcons ()
	{
		...
		if (compassEnabled) {
			compassDirections.rotation = Quaternion.identity;
			compassDirections.transform.position = player.transform.position;

			playerCompassPosition = new Vector3 (player.transform.position.x, 0, player.transform.position.z);

			for (int i = 0; i < compassIconList.Count; i++) {

				if (compassIconList [i].iconGameObject) {

					compassTargetPosition = new Vector3 (compassIconList [i].iconTargetTransform.position.x, 0, compassIconList [i].iconTargetTransform.position.z);

					compassDirection = compassTargetPosition - playerCompassPosition;
					compassDirection = compassDirection / compassDirection.magnitude;
					currentCompassTargetRotation = Vector3.SignedAngle (compassDirection, player.transform.forward, Vector3.up);

					compassTargetRotation = currentCompassTargetRotation;
					//print (currentCompassTargetRotation);
					...
					Vector2 newPosition = new Vector2 ((-currentCompassOrientation * 2 + currentCompassTargetRotation) * compassScale, compassIconList [i].verticalOffset);

					if (compassIconList [i].useCompassClamp) {
						newPosition.x = Mathf.Clamp (newPosition.x, maximumLeftDistance, maximumRightDistance);
					}

					compassIconList [i].iconRectTransform.anchoredPosition = newPosition;
						;
				} else {
	...
	{
		if (mapObjects [index].compassIcon != null) {
			Destroy (mapObjects [index].compassIcon.iconGameObject);
		}
	}
	...
	IEnumerator LerpFromTo (Matrix4x4 src, Matrix4x4 dest, float duration)
	{
		...
		for (float t = 0; t < duration;) {
			t += Time.deltaTime * transtionTo3dSpeed;
		...
		if (!mapViewIn2dActive) {
			mapSystemPivotTransform.position = mapCamera.transform.position - Vector3.up * cameraOffset.y;
	                          ...
	                          bool setCustomCompassSettings, bool useCompassIcon, GameObject compassIconPrefab, float verticalOffset)
	{
		...
		for (int i = 0; i < mapObjects.Count; i++) {
			if (mapObjects [i].mapObject == obj) {
				...
				if (newMapObject != null && newMapObject.mapIcon) {
		...
		for (int i = 0; i < mapIconTypes.Count; i++) {
			if (mapIconTypes [i].typeName == type) {
				...
				if (setCustomCompassSettings && useCompassIcon) {
					GameObject newCompassIcon = (GameObject)Instantiate (compassIconPrefab, Vector3.zero, Quaternion.identity);
					newCompassIcon.transform.SetParent (compassElementsParent.transform);
					...
					if (mapIconTypes [i].useCompassIcon) {
						GameObject newCompassIcon = (GameObject)Instantiate (mapIconTypes [i].compassIconPrefab, Vector3.zero, Quaternion.identity);
						newCompassIcon.transform.SetParent (compassElementsParent.transform);
						newCompassIcon.transform.localScale = Vector3.one;

						newMapObject.compassIcon = addCompassIcon (obj, newCompassIcon, mapIconTypes [i].verticalOffset, true);
					}
				}
			}
		}
		...
		if (!alreadyAdded) {
			...
			if (currentMapObjectInformation) {
				...
				newMapObject.name = currentMapObjectInformation.name;
			...
			if (calculateFloor) {
				...
				if (buildingIndex < buildingList.Count) {
					...
					for (int i = 0; i < currentFloorsToCheck.Count; i++) {
			...
			if (mapObjects [i].mapObject == obj) {
				...
				if (mapObjects [i].typeName == "Mark") {
					if (mapObjects [i].mapObject) {
						Destroy (mapObjects [i].mapObject);	
					}
				}
	...
	public GameObject addMapTextIcon (mapObjectInfo newMapObject)
	{
		...
		GameObject textIcon = (GameObject)Instantiate (mapObjectTextIcon, newMapObject.mapIcon.position, Quaternion.identity);
		textIcon.transform.SetParent (newMapObject.mapIcon);
	...
	public void removeMark ()
	{
		if (currentMark != null) {
			for (int k = 0; k < markList.Count; k++) { 
				if (markList [k].mapObject == currentMark.mapObject) {
					markList.RemoveAt (k);
				}
			}
	...
	{
		for (int i = 0; i < mapObjects.Count; i++) {
			if (mapObjects [i].mapObject == objectToSearch) {
				mapObjects [i].floorNumber = newFloorIndex;
			}
		}
	}
	...
	public void changeMapObjectIconFloorByPosition (GameObject objectToSearch)
	{
		...
		for (int i = 0; i < mapObjects.Count; i++) {
			if (mapObjects [i].mapObject && mapObjects [i].mapObject == objectToSearch) {

				int newFloorNumber = getClosestFloorToMapObject (mapObjects [i].mapObject.transform);
				if (newFloorNumber > -1) {
					mapObjects [i].floorNumber = floors [newFloorNumber].floorNumber;
				}
			}
		}
	}
	...
	{
		for (int i = 0; i < mapObjects.Count; i++) {
			if (mapObjects [i].typeName == typeName) {
				if (!value || (value && mapObjects [i].mapPartOwnerFound)) {
					mapObjects [i].isActivated = value;
				}
			}
		}
		int sliderValue = 0;
		...
		for (int i = 0; i < glossaryList.Count; i++) {
			if (glossaryList [i].typeName == typeName) {
	...
	{
		for (int i = 0; i < mapObjects.Count; i++) {
			if (mapObjects [i].mapPartOwnerFound && mapObjects [i].buildingIndex == currentBuildingIndex) {
				mapObjects [i].canBeShowTemporaly = value;
			}
		}
	}
	...
	public void enableOrDisableSingleMapIconByID (int ID, bool value)
	{
		for (int i = 0; i < mapObjects.Count; i++) {
			if (mapObjects [i].ID == ID) {
				mapObjects [i].isActivated = value;
				mapObjects [i].mapPartOwnerFound = value;
			}
		}
	}
	...
	{
		for (int i = 0; i < mapObjects.Count; i++) {
			if (mapObjects [i].mapPartIndex == mapPartIndex && mapObjects [i].floorIndex == mapFloorIndex && mapObjects [i].buildingIndex == mapPartBuildingIndex) {
				mapObjects [i].isActivated = value;
				mapObjects [i].mapPartOwnerFound = value;
			}
		}
	...
	{
		for (int i = 0; i < mapObjects.Count; i++) {
			if (mapObjects [i].useEventsOnChangeFloor && mapObjects [i].hasMapObjectInformation) {
				mapObjects [i].mapObjectInformationManager.checkEventOnChangeFloor (currentBuildingIndex, currentFloorIndex);
			}
		}
	}
	...
	{
		mapObjectInformation currentMapObjectInformation = mapObjects [mapObjectIndex].mapObject.GetComponent<mapObjectInformation> ();
		if (currentMapObjectInformation) {
			mapObjectNameField.text = currentMapObjectInformation.name;
			mapObjectInfoField.text = currentMapObjectInformation.description;

			if (currentMapObjectInformation.typeName == "Beacon") {
				quickTravelButtonImage.color = originalQuickTravelColor;

				currentQuickTravelStation = mapObjects [mapObjectIndex].mapObject;
			} else {
				quickTravelButtonImage.color = disabledQuickTravelColor;
			}

			if (mapObjects [mapObjectIndex].typeName == "Mark") {
				currentMark = mapObjects [mapObjectIndex];
	...
	public void activateQuickTravel ()
	{
		if (currentQuickTravelStation) {
			...
			if (playerMapObjectInformation && quickTravelStationMapObjectInformation) {
	...
	{
		if (mapEnabled && (!pauseManager.playerMenuActive || mapOpened) && !playerControllerManager.isUsingDevice () && !mainGameManager.gamePaused) {
			mapOpened = state;
			...
			pauseManager.setIngameMenuOpenedState ("Map System", mapOpened);

			if (mapOpened) {
				currentIconSizeMultiplier = openMapIconSizeMultiplier;
				targetMapPosition = mapWindowTargetPosition.position;
			...
			playerControllerManager.changeScriptState (!mapOpened);
			pauseManager.usingSubMenuState (mapOpened);

			pauseManager.enableOrDisableDynamicElementsOnScreen (!mapOpened);

			pauseManager.showOrHideMouseCursorController (mapOpened);
			...
			if (useMapCursor && mapCursor) {
				mapCursor.SetActive (mapOpened);
				if (mapOpened) {
	...
	IEnumerator changeMapPositionCoroutine ()
	{
		...
		if (miniMapWindowEnabledInGame) {
			...
			mapWindow.gameObject.SetActive (mapOpened);
			mapWindow.position = targetMapPosition;
			mapWindow.sizeDelta = targetScale;
			mapRender.sizeDelta = targetScale;
			changeMapWindowParent = true;
		...
		if (changeMapWindowParent) {
			if (mapOpened) {
				mapWindow.SetParent (mapWindowTargetPosition.parent);
			} else {
				mapWindow.SetParent (mapMenu.transform.parent);
			}
		}
		...
		if (usingCircleMap) {
			if (mapOpened) {
				...
				mapRender.SetParent (mapCircleTransform);
			}
	...
	public void enableOrDisableMiniMap (bool state)
	{
		if (state) {
			if (miniMapWindowEnabledInGame) {
				mapWindow.gameObject.SetActive (true);
				if (compassEnabled) {
					compassWindow.gameObject.SetActive (true);
				}
			}
		} else {
			mapWindow.gameObject.SetActive (false);
	...
	public int getClosestFloorToPlayer ()
	{
		...
		for (int i = 0; i < floors.Count; i++) {
			if (floors [i].floor) {
				float currentDistance = Mathf.Abs (player.transform.position.y - floors [i].floor.position.y);
				if (currentDistance < distance) {
					distance = currentDistance;
					index = i;
				}
	...
	public void updateCurrentFloorNumber ()
	{
		...
		if (floors.Count > currentFloorIndex) {
			...
			print ("WARNING: the system is trying to acces to a floor index too high to the current building, make sure the triggers to change floor and building are placed properly");
		}
...
//		}
	...
	public void updateCurrentBuildingNumber ()
	{
		...
		if (currenttBuilding.useCameraPositionOnMapMenu) {
			Vector3 newPosition = currenttBuilding.cameraPositionOnMapMenu.position;

			if (mapViewIn2dActive) {

				mainMapCameraTransform.rotation = currenttBuilding.cameraPositionOnMapMenu.rotation;

				if (currenttBuilding.useCameraOffset) {
				...
				mapCamera.transform.localPosition = Vector3.up * cameraOffset.y;
...
//		}
	...
	public void updateCurrentBuilding (int buildingIndex)
	{
		...
		currenttBuilding = buildingList [buildingIndex];
		floors = currenttBuilding.floors;

		currentFloorIndex = getClosestFloorToPlayer ();

		currentBuildingIndex = buildingIndex;
		currentBuildingName = currenttBuilding.Name;
	...
	{
		currentMapZoneText.text = currentBuildingName.ToUpper () + " - " + floors [currentFloorIndex].Name.ToUpper ();
	}
	...
	public void searchBuildingList ()
	{
		...
		if (mapCreatorManager) {
			...
			for (int i = 0; i < mapCreatorManager.buildingList.Count; i++) {
				...
				newBuildingInfo.isCurrentMap = mapCreatorManager.buildingList [i].isCurrentMap;
				newBuildingInfo.isInterior = mapCreatorManager.buildingList [i].isInterior;

				newBuildingInfo.buildingFloorsParent = mapCreatorManager.buildingList [i].buildingFloorsParent;

				newBuildingInfo.useCameraPositionOnMapMenu = mapCreatorManager.buildingList [i].useCameraPositionOnMapMenu;
				newBuildingInfo.useCameraOffset = mapCreatorManager.buildingList [i].useCameraOffset;
				if (newBuildingInfo.useCameraPositionOnMapMenu) {
					newBuildingInfo.cameraPositionOnMapMenu = mapCreatorManager.buildingList [i].cameraPositionOnMapMenu;
				}
				...
				for (int j = 0; j < mapCreatorManager.buildingList [i].buildingFloorsList.Count; j++) {
					...
					newFloorInfo.floorNumber = mapCreatorManager.buildingList [i].buildingFloorsList [j].floorNumber;
					newFloorInfo.floor = mapCreatorManager.buildingList [i].buildingFloorsList [j].floor.transform;
					newBuildingInfo.floors.Add (newFloorInfo);
				}

				if (newBuildingInfo.isCurrentMap) {
					floors = newBuildingInfo.floors;
					currenttBuilding = newBuildingInfo;
			...
			mapCreatorManager.addNewPlayer (this, player);
		} else {
	...
	public void getEditorBuildingList ()
	{
		if (buildingList.Count > 0) {
			...
			print ("To disable this message in editor time, set to false the field Update Map Info Every Frame in the Map System inspector");
		}
	}

	public bool updateMapInfoEveryFrame;
	...
	public void getEditorMapPartList ()
	{
		...
		if (mapCreatorManager) {
			if (currentFloorIndex >= 0 && currentBuildingIndex >= 0) {
				if (currentBuildingIndex < mapCreatorManager.buildingList.Count && currentFloorIndex < mapCreatorManager.buildingList [currentBuildingIndex].buildingFloorsList.Count) {
					mapPartListString = new string[mapCreatorManager.buildingList [currentBuildingIndex].buildingFloorsList [currentFloorIndex].mapPartsList.Count];
					for (int i = 0; i < mapCreatorManager.buildingList [currentBuildingIndex].buildingFloorsList [currentFloorIndex].mapPartsList.Count; i++) {
						string name = mapCreatorManager.buildingList [currentBuildingIndex].buildingFloorsList [currentFloorIndex].mapPartsList [i].name;
