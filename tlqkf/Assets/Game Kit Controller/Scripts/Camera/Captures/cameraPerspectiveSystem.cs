...
public class cameraPerspectiveSystem : MonoBehaviour
{
		...
		if (!captureTakenCorrectly) {
			if (playerInside && useProximitySound) {
				if (Time.time > lastTimePlayed + soundRate) {
					...
					bipAudioSource.PlayOneShot (proximitySound);
		...
		if ((1 << col.gameObject.layer & layerForUsers.value) == 1 << col.gameObject.layer) {
			...
			if (isEnter) {
				...
				if (smartphoneDeviceManager == null) {
					smartphoneDeviceManager = currentPlayer.GetComponent<playerController> ().getPlayerCameraGameObject ().GetComponentInChildren<smartphoneDevice> ();
				}
				if (smartphoneDeviceManager) {
					smartphoneDeviceManager.setCurrentPerspectiveSystem (GetComponent<cameraPerspectiveSystem> ());
				}
	...
	{
		if (showGizmo) {
			if (positionToStay && positionToLook) {
				...
				GKC_Utils.drawGizmoArrow (positionToStay.position, direction * distance, gizmoArrowColor, gizmoArrowLenght, gizmoArrowAngle);
